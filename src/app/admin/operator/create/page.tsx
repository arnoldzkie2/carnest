...
const Page = () => {
  ...
  const createOperator = async () => {
    ...
    try {
      ...
      setIsLoading(true)
      const { data } = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/signup/operator`, formData, {
        headers: {
  ...
  return (
    <>
      ...
      <div className='py-28 px-5 sm:px-10 md:px-16 lg:px-24 xl:px-36 2xl:px-44 flex flex-col items-center gap-5'>
        <h1 className='text-2xl text-slate-800 w-full text-center'>Create Operator</h1>
        <form className='flex flex-col gap-5 w-full md:w-1/2 md:border md:p-10 md:shadow-lg' onSubmit={createOperator}>
