...
const Page = ({ params }: Props) => {
  ...
  const retrieveUser = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/user?id=${params.userID}`, {
        headers: {
  ...
  const updateUser = async () => {
    ...
    try {
      ...
      const { data } = await axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/user`, {
  ...
  return (
    <>
      ...
      <div className='py-28 px-5 sm:px-10 md:px-16 lg:px-24 xl:px-36 2xl:px-44 flex flex-col items-center gap-5'>
        <h1 className='text-2xl text-slate-800 w-full text-center'>Update User</h1>
        <form className='flex flex-col gap-5 w-full md:w-1/2 md:border md:p-10 md:shadow-lg' onSubmit={updateUser}>
