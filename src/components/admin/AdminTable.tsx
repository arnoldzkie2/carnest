...
const AdminTable = () => {
    ...
    const { allAdmin, getAllAdmin, getAdminToken, openAdminModal, toggleNewAdmin } = useAdminStore()
    ...
    const searchedAdmin = allAdmin.filter(admin => admin.name.toUpperCase().includes(searchQuey.toUpperCase()))

    const filteredTable = searchedAdmin.slice(startIndex, endIndex);

    const totalPages = Math.ceil((allAdmin.length || 0) / itemsPerPage);
    ...
    const deleteAdmin = async (e: any, ID: number) => {
    ...
    return (
        <div className='pt-32 pb-10 px-5 h-screen sm:px-10 md:px-16 lg:px-24 xl:px-36 2xl:px-44 flex flex-col w-full items-center'>
            <div className='flex items-center w-full md:w-4/5'>
                ...
                }} type="text" placeholder='Search Admin (name)' className='py-2.5 px-4 w-full border outline-none' />
                <li onClick={toggleNewAdmin} className='bg-red-700 cursor-pointer hover:bg-red-500 text-white w-40 flex items-center justify-center py-2.5'>New Admin</li>
            </div>
                                ...
                                filteredTable.map(admin => (
                                    <tr className="bg-white border hover:bg-slate-50" key={admin.id}>
                                        ...
                                        <td className='py-3 relative px-6'>
                                            <FontAwesomeIcon icon={faEllipsis} width={16} height={16} className='text-xl cursor-pointer text-black' onClick={() => openOperation(admin.id)} />
                                            <ul className={`${operation && selectedID === admin.id ? 'block' : 'hidden'} absolute bg-white p-3 gap-1 z-20 w-24 shadow-lg border flex flex-col text-gray-600`}>
                                                <li onClick={() => openAdminModal(admin)} className='flex mb-1 justify-between items-center cursor-pointer hover:text-blue-500'>Update <FontAwesomeIcon icon={faPenSquare} /></li>
                                                <button disabled={isLoading} onClick={(e: any) => deleteAdmin(e, admin.id)} className='flex mb-1 justify-between items-center cursor-pointer hover:text-red-500'>{isLoading ? <FontAwesomeIcon icon={faSpinner} className='animate-spin' /> : 'Delete'}{!isLoading && <FontAwesomeIcon icon={faTrashCan} />}</button>
