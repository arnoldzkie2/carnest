...
const UserTable = () => {
    ...
    const { users, getAdminToken, getUsers } = useAdminStore()
    ...
    const searchedUser = users.filter(user => user.name.toUpperCase().includes(searchQuey.toUpperCase()))
    ...
    return (
        <div className='pt-32 pb-10 px-5 h-screen sm:px-10 md:px-16 lg:px-24 xl:px-36 2xl:px-44 flex flex-col w-full items-center'>
            <div className='flex items-center w-full md:w-4/5'>
                <Link href={'/admin/user/create'} className='bg-red-700 cursor-pointer hover:bg-red-500 text-white w-40 flex items-center justify-center py-2.5'>New User</Link>
                ...
                <Link href={'/admin/pending/user'} className='bg-red-700 cursor-pointer hover:bg-red-500 text-white w-40 flex items-center justify-center py-2.5'>Pending</Link>
            </div>
                        ...
                        <tbody>
                            {filteredTable && filteredTable.length > 0 ?
                                filteredTable.map(user => (
                                    <tr className="bg-white border hover:bg-slate-50" key={user.id}>
                                        <td className='px-6 py-3'>
                                            <div className='h-5 w-36 flex items-center gap-3'>
                                                <Image src={user.profile || '/default-profile.png'} width={20} height={20} alt='User Profile' className='rounded-full' />
                                                {user.name}
                                            </div>
                                        </td>
                                        <td className="px-6 py-3">
                                        ...
                                        <td className='py-3 relative px-6'>
                                            <FontAwesomeIcon icon={faEllipsis} width={16} height={16} className='text-xl cursor-pointer text-black' onClick={() => openOperation(user.id)} />
                                            <ul className={`${operation && selectedID === user.id ? 'block' : 'hidden'} absolute bg-white p-3 gap-1 z-20 w-24 shadow-lg border flex flex-col text-gray-600`}>
                                                <Link href={`/admin/user/visit/${user.id}`} className='flex mb-1 justify-between items-center cursor-pointer hover:text-green-500'>View <FontAwesomeIcon icon={faEye} /></Link>
                                                <Link href={`/admin/user/update/${user.id}`} className='flex mb-1 justify-between items-center cursor-pointer hover:text-blue-500'>Update <FontAwesomeIcon icon={faPenSquare} /></Link>
                                                <button disabled={isLoading} onClick={(e: any) => deleteUser(e, user.id)} className='flex mb-1 justify-between items-center cursor-pointer hover:text-red-500'>{isLoading ?
                                                    <FontAwesomeIcon icon={faSpinner} className='animate-spin' /> : 'Delete'}{!isLoading && <FontAwesomeIcon icon={faTrashCan} />}</button>
