    ...
    return (
        <div className='pt-32 pb-10 px-5 h-screen sm:px-10 md:px-16 lg:px-24 xl:px-36 2xl:px-44 flex flex-col w-full items-center'>
            <div className='flex items-center w-full md:w-4/5'>
                ...
                <Link href={'/admin/pending/operator'} className='bg-red-700 cursor-pointer hover:bg-red-500 text-white w-40 flex items-center justify-center py-2.5'>Pending</Link>
            </div>
                                ...
                                filteredTable.map(operator => (
                                    <tr className="bg-white border hover:bg-slate-50" key={operator.id}>
                                        ...
                                        <td className='py-3 relative px-6'>
                                            <FontAwesomeIcon icon={faEllipsis} width={16} height={16} className='text-xl cursor-pointer text-black' onClick={() => openOperation(operator.id)} />
                                            <ul className={`${operation && selectedID === operator.id ? 'block' : 'hidden'} absolute bg-white p-3 gap-1 z-20 w-24 shadow-lg border flex flex-col text-gray-600`}>
                                                <Link href={`/admin/operator/visit/${operator.id}`} className='flex mb-1 justify-between items-center cursor-pointer hover:text-green-500'>View <FontAwesomeIcon icon={faEye} /></Link>
                                                <Link href={`/admin/operator/update/${operator.id}`} className='flex mb-1 justify-between items-center cursor-pointer hover:text-blue-500'>Update <FontAwesomeIcon icon={faPenSquare} /></Link>
                                                <button disabled={isLoading} onClick={(e: any) => deleteOperator(e, operator.id)} className='flex mb-1 justify-between items-center cursor-pointer hover:text-red-500'>{isLoading ?
