...
const SignupOperator = () => {
    ...
    const registerOperator = async (e: any) => {
        ...
        try {
            ...
            const { data } = await axios.post(`${process.env.NEXT_PUBLIC_API_BACKEND}/signup/operator`, {

                operator: { name, email, password }
    ...
    return (
        <div className="flex-col h-screen justify-center lg:py-36 gap-7 px-5 sm:px-10 md:px-16 lg:px-24 xl:px-36 2xl:px-44 flex items-center">
            <h1 className='text-slate-700 text-center text-3xl lg:text-5xl xl:text-6xl md:text-4xl font-light lg:font-extralight'>Operator Signup</h1>
            <h2 className='text-slate-600 text-center leading-6 md:leading-8 text-sm md:text-base'>Sign Up as an Operator to Offer Your Cars on CarNest</h2>

            <form onSubmit={registerOperator} className='w-full flex flex-col gap-5 text-slate-600 sm:p-6 sm:border sm:w-[26rem] md:w-[28rem] md:p-10 xl:w-[30rem] xl:p-12 xl:gap-6'>
            ...
            <div className='text-slate-600 flex items-center gap-3'>
                ...
                <Link href={'/login/operator'} className='text-red-700 hover:text-red-500'>Login</Link>
            </div>
