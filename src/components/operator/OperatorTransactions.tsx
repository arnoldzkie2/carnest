...
const OperatorTranscactions = () => {

    const { operator, getOperatorToken } = useOperatorStore()

    const [booking, setBooking] = useState<Booking[]>([])

    const { operation, selectedID, openOperation, viewBooking, closeOperation, isLoading, setIsLoading, itemsPerPage, currentPage, setCurrentPage } = useGlobalStore()
    ...
    const confirmBooking = async (e: any, bookingID: number) => {
    ...
    return (
        <div className='flex flex-col gap-10 lg:gap-20 text-slate-600 w-full md:flex-row py-24 lg:py-36 px-5 sm:px-10 md:px-16 lg:px-24 xl:px-36 2xl:px-44'>
            ...
            <div className='flex flex-col md:gap-10 items-center md:items-start w-full md:w-1/2'>
                <div className='flex flex-col gap-5 items-center'>
                    <div className='flex flex-col gap-5 w-full md:flex-row md:items-center md:border-b-2 md:pb-5'>
                        ...
                        <Image
                            src={operator?.profile ? operator?.profile : '/default-profile.png'}
                ...
                {operator?.status === 'pending' ? <div className='hidden md:flex'>Your Account is Pending</div> : <ul className='flex-col gap-2 md:flex hidden w-full'>
                    <h1 className='text-xl text-red-700 border-b pb-2'>Account</h1>
                    <Link href={'/operator/profile'} className='hover:text-red-700'>
                        My Profile
                    </Link>
                </ul>}
            </div>
            ...
            <div className='w-full flex flex-col gap-5'>
                ...
                <div className='w-full h-full overflow-x-auto pb-28 pr-10'>
                                ...
                                filteredTable.map(transac => (
                                    <tr className="bg-white border hover:bg-slate-50" key={transac.id}>
                                        ...
                                        <td className="px-6 py-3 flex items-center">
                                            <div className='h-5 w-40 flex items-center'>
                                                {transac.status === 0 ? 'Canceled' : transac.status === 1 ? 'Admin Confirmation' : transac.status === 2 ?
                                                    <button onClick={(e: any) => confirmBooking(e, transac.id)} className={`${isLoading ? 'bg-red-500' : 'bg-red-700 hover:bg-red-500'} px-6 py-1.5 rounded-md text-white`}>{isLoading ?
                                                        <FontAwesomeIcon icon={faSpinner} className='animate-spin' width={16} height={16} /> : 'Confirm'}</button> : 'Confirmed'}
                                            </div>
                                        </td>
...
export default OperatorTranscactions