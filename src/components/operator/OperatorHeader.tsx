...
const OperatorHeader = () => {
    ...
    const { logoutOperator, getOperator, operator } = useOperatorStore()
    ...
    return (
        <header className='z-20 bg-white px-5 sm:px-10 md:px-16 border-b lg:px-24 xl:px-36 2xl:px-44 h-16 md:h-20 fixed w-screen flex items-center top-0 left-0 justify-between'>
            <Link href={'/operator'} className={`text-red-700 flex items-center text-2xl md:text-3xl font-black`}>
                CARNEST
            </Link>
            ...
            <ul className={`w-full lg:flex lg:items-center xl:gap-10 lg:gap-5 ${isOpen ? 'gap-3 shadow-2xl flex flex-col fixed top-0 left-0 w-screen bg-white px-5 sm:px-10 md:px-16 md:pt-[8.7px] pb-5 pt-[8.2px] text-gray-700' : 'hidden'}`}>
                <Link href={'/operator'} className='flex mt-4 lg:hidden items-center text-2xl font-black text-red-700'>
                    CARNEST
                </Link>
                <Link href='/operator' className={`lg:ml-auto rounded-2xl cursor-pointer hover:text-red-700`}>Dashboard</Link>
                <Link href='/operator/profile' className={`rounded-2xl cursor-pointer hover:text-red-700`}>Profile</Link>
                <div className={`${isOpen ? 'mt-5' : ''} flex items-center gap-5  lg:ml-auto`}>
                    <Link href={'/operator/profile'} className='flex items-center gap-2 border px-3 py-1'>
                        <h1>{operator?.name}</h1>
                        <Image
